...
void RigidBodySystem::step(float dt)
{
    ...
    m_collisionDetect->detectCollisions_fcl();
...
#pragma omp for schedule(static)
        for (int bi = 0; bi < m_bodies.size(); bi++)
        {
            RigidBody* b = m_bodies[bi];
            if (b->fixed) {
...
void RigidBodySystem::step_semi(float dt)
{
    // Initialize the system.
    // Apply gravitional forces and reset angular forces.
    // Cleanup contacts from the previous time step.
    ...
    for (RigidBody* b : m_bodies)
    {
        ...
        // velocity
        b->xdot += dt * (1.0f / b->mass) * (b->fc);
        b->omega += dt * b->Iinv * (b->tauc - b->omega.cross(b->I * b->omega));
